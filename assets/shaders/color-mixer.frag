...
void main(){
    ...
    // here we use the dot function to mix the color channels
    float mixColor = dot(frag_color.rgb, fs_in.color);
    // we then set the fragment color to the mixed color
    frag_color.rgb = vec3(mixColor);
}